...
open_set(startPointData(3)) = startPointData(3); % only store the ID not the whole point
...
all_pts_plus_start_and_fin = [pointsWithData; startPointData; finishPointData];


% find all heuristic costs as distances from point to finish
heuristicCost = sqrt((all_pts_plus_start_and_fin(:,1) - finishPointData(1)).^2 + (all_pts_plus_start_and_fin(:,2) - finishPointData(2)).^2)';
