function [cost, route] = fcn_Greedy_greedyPlanner(visibilityMatrix, pointsWithData, startPointData, finishPointData, varargin)
...
% INPUTS:
%
%   visibilityMatrix: the nxn visibility graph of the map, where n is the total
...
%
%   startPointData: the 1x5 vector describing the start point, with the same
%   information as pointsWithData
%
%   finishPointData: the 1x5 vector describing the finish point, with the same
%   information as pointsWithData
%
%   (optional inputs)
%
...
if flag_do_debug
    ...
    h_polytopes = fcn_MapGen_plotPolytopes(polytopes,(plotFormat),(fillFormat),(figNum));
...
numNodes = size(visibilityMatrix,1); % number of nodes in the cost graph

% Grab start and end point
startPointData = pointsWithData(end-1,:);
finishPointData = pointsWithData(end,:);

visibilityMatrixNoSelfVisibility = visibilityMatrix - eye(numNodes);

% 1.  Initialize the open list: put the starting node on the open list
%     (we can leave its f at zero)

% Open set is the set of all nodes that have been visited thus far. This
% starts out as NaN values except for the start point.
open_set = nan(1,numNodes);
startPointID = startPointData(3);
open_set(startPointID) = startPointID; % only store the ID not the whole point

% The g-value for the current open set selection is the "row" of the cost
% matrix.
open_set_gs = inf*ones(1,numNodes); % initialize costs of open set to infinity
open_set_gs(startPointData(3)) = 0; % g-value for nodes in open set.  g is the movement cost to

% find all heuristic costs as distances from point to finish
% 1 row, many columns?
heuristicCost = sqrt((pointsWithData(:,1) - finishPointData(1)).^2 + (pointsWithData(:,2) - finishPointData(2)).^2)';
...
open_set_fs = open_set_gs + heuristicCost; % f-vlaue for nodes in the open set.
...
while any(isnan(open_set))
    % a) find the node with the least cost, f, on the open list. Call it "q"
    [~, idx_of_q] = min(open_set_fs);
    q = pointsWithData(idx_of_q,:);
    q_history = [q_history; q]; %#ok<AGROW>
    ...
    qs_row = visibilityMatrixNoSelfVisibility(idx_of_q,:);

    successor_idxs = find(qs_row);
    % TODO @sjharnett set parents to q
    q_parents{end+1} = successor_idxs; %#ok<AGROW>

    % d) for each successor
    for i = 1:length(successor_idxs)
        successor = pointsWithData(successor_idxs(i),:);
